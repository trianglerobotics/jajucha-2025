...
const WLAN_SECURITY_ATTRIBUTES = StructType({
  bSecurityEnabled: DWORD,         // BOOL → DWORD (4 bytes)
  bOneXEnabled: DWORD,             // BOOL → DWORD (4 bytes)
  dot11AuthAlgorithm: DWORD,       // DOT11_AUTH_ALGORITHM (4 bytes)
  dot11CipherAlgorithm: DWORD      // DOT11_CIPHER_ALGORITHM (4 bytes)
}, { packed: true }); // ✅ 총 16 bytes
...
function getWifiInterfacesList(wlanHandle) {
  ...
  const numInterfaces = interfaceList.dwNumberOfItems;

  // 🛠 Fix: Reinterpret memory correctly
  const interfaceArrayBuffer = ref.reinterpret(interfaceList.InterfaceInfo.buffer, numInterfaces * WLAN_INTERFACE_INFO.size, 0);
  const interfaceArray = Array.from({ length: numInterfaces }, (_, i) =>
    WLAN_INTERFACE_INFO.get(interfaceArrayBuffer, i * WLAN_INTERFACE_INFO.size)
  );

  return interfaceArray.map((iface, i) => ({
    index: i,
...
function getInterfaceConnectedWifiInfo(wlanHandle: any, ifaceGuid: any): WifiInfo {
  ...
  const queryResult = wlanapi.WlanQueryInterface(wlanHandle, ifaceGuidPtr, WLAN_CONNECTION_OPCODE, ref.NULL, dataSizeRef, dataRef, wlanOpcodeValueType);

  if (queryResult !== 0) {
    console.error(`WlanQueryInterface failed with error code: ${queryResult}`);
    return { ssid: "Unknown", signalQuality: -1, rxRate: 0, txRate: 0 };
  }

  if (dataRef.isNull()) {
    console.error("Error: dataRef is NULL, no data returned from WlanQueryInterface");
    return { ssid: "Unknown", signalQuality: -1, rxRate: 0, txRate: 0 };
  }

  const dataSize = dataSizeRef.deref();
  if (dataSize < WLAN_CONNECTION_ATTRIBUTES.size) {
    console.error(`Error: Expected data size ${WLAN_CONNECTION_ATTRIBUTES.size}, but got ${dataSize}`);
    return { ssid: "Unknown", signalQuality: -1, rxRate: 0, txRate: 0 };
  }
